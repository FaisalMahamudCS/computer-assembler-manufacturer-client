...
const Blog = () => {
    return (
        <div className='m-3'>
           <h3 className='text-xl font-bold'>How can we improve performance of React Application?</h3> 
...
of index as key it cause performance issue
           ...
           <h3  className='text-xl font-bold'>Different way to manage state in react application</h3>
           <p>
       ...
       Query Library and SWR library.URL state is used data which is exist in url.we have to rely on accessing url state
            ...
            <h3  className='text-xl font-bold'>How prototypical inheritance Work</h3>
       ...
       <p>We donot set state directly.because if we set state directly we will loose control of product states.if we can directly set is as product=
           ...
           products = [...]  value and because of directly updating state ,it doesn't change.if we directly do it 
           it doesn't change the product state immediately.it will generate a pending state transition which will yeild
           the current value .if we directly change it we will loose control of state in all of the component where product is used.
           </p>
           <h3  className='text-xl font-bold'>Why do we use unit testing?</h3>
           <p>We use unit testing to ensure the quality standard of all code we written.
               ...
               in production it would be hard to find it.unit testing help to code efficiently.the main perpous of
               it is to check all possible parts of software.In unit testing tester make the test case.then he test  and 
               execute test case
           </p>
        </div>
    );
